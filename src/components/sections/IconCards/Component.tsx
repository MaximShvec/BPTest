...
export function IconCards({ title, compact = false, items }: IconCardsData) {
  return (
      ...
      <Grid>
        {items.map((item) => (
          <GridItem key={item.title} span={item.span ?? 4} spanTablet={item.spanTablet} spanMobile={item.spanMobile ?? 12}>
            <article
              className={cx(styles.card, styles[item.theme ?? "white"], compact && styles.compact)}
            ...
            >
              ...
              {compact ? <h3 className={styles.compactTitle}>{item.title}</h3> : <h3 className={styles.title}>{item.title}</h3>}
