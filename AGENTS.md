...
- Контраст: исходный `#939393` (`--gray300`) на `#f3f5f5` даёт 2.8:1, на белом 3.07:1. Подписи `.caption` могут оставаться около 3:1, более мелкий текст — нет. В проблемных местах (caption, подписи плейсхолдеров, лицензии, приглушённые строки продуктов) используется `--gray300-a11y: #7a7a7a`. Остальной серый текста макета не менять.
